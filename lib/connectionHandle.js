...
ConnectionHandle.prototype.openAMQPCommunication = function() {
	...
	var heartbeat = true;
	var vhost = '/';
	var validateProps = _validateProps;
	var callback;
	...
	async.series([
			function(seriesCallback) {
				self.once('connection.start', function(channel, method, data) {
					var clientProps = validateProps(data);
					if (clientProps.error) {
						return callback(clientProps.error);
					}
...
function _validateProps(serverProps) {

	/* Default RabbitMQ capabilities */
	...
	var serverCapabilities = serverProps['server-properties']['capabilities'];
	for (var capability in clientCapabilities) {
	   if (clientCapabilities[capability].data && !serverCapabilities[capability]) {
		   return { error: 'client-server capability mismatch: ' + capability };
	   }
	}
	return clientProps;
};
